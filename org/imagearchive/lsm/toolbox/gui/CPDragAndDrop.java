			...
			if (t.isDataFlavorSupported(DataFlavor.javaFileListFlavor)) {
				...
				while(iterator.hasNext()) {
					...
					for (int i = 0; i < imp.length; i++) {
						imp[i].show();
						// imp[i].getWindow().addWindowListener(new
						// ImageFocusListener(masterModel,
						// (LsmFileInfo)imp[i].getOriginalFileInfo(),
						// imp[i]));
						iwc = imp[i].getWindow();
						// if (iwc != null) {
						// if (iwc.getImagePlus().getOriginalFileInfo()
						// instanceof LsmFileInfo) {
					...
					cp.infoFrame.updateInfoFrame(masterModel.getInfo());
