 ...
 *
 * -----
 * This version (modified by Mark Longair) has a few changes, notably
 * an Open method that returns the channels as an array of ImagePlus.
 *
 * FIXME: check whether this is provide by the new version anyway
 * FIXME: check what other changes I made
 * 
 * -----
 *
 ...
 *
 * Emails    :      Project maintainers          :patrick.pirrotte@gmx.net
 *                                                jerome.mutterer@ibmp-ulp.u-strasbg.fr
 *                                    yannick.krempp@iphysiol.unil.ch
 *
 ...
 *
 *
 * 2005/07/21 (v3.2f):  -Adds support for time events detection. There is now a time event window that opens together with the image if it is a time series,
 ...
 *
 *
 * 2005/07/19 (v3.2e):  Works with ImageJ 1.34s
 *
 *
 ...
 *          into various usual file format : tiff, 8-bit tiff, jpeg, raw, zip. This converter handles also stacks and lambda stacks by  *           saving one image for each slice of the stack. Credits for the base batch conversion method goes for Wayne Rasband,
...
public class LSM_Reader implements PlugIn{
    ...
    /* MHL - took this out of the old RandomAccessStream */

    // readLine() is a really terrible interface, ignoring character
    // set consideration, and so on...

    public static String readLine(RandomAccessStream stream) throws IOException {
        ...
        while( true ) {
            ...
            if( readResult == '\r' ) {
                // Peek ahead to see if the next is a '\n' ...
                int oldPosition = stream.getFilePointer();
                int readNextResult = stream.read();
                if( readResult == '\n' ) {
                    break;
            ...
            result = result.append( (char)readResult );
        }
        if( hitEOF && ( result.length() == 0 ) )
            return null;
        else
    ...
    public ImagePlus[] OpenLSM2(String directory,String name) {
        LSM_Reader_ reader = new LSM_Reader.LSM_Reader_();
        return reader.OpenLSM2( directory, name );        
    }

    public void run(String args) {

        // System.out.println( "LSM_Reader:run() with '" + args + "'" );

        if (args.equals("about")){
            IJ.showMessage("LSM Reader 3.2(f)",
                    "Copyright (C) 2002-2004 P. Pirrotte, Y. Krempp & J. Mutterer\n"
                    +" \n"
        ...
        if (args != ""){
            ...
            // FIXME: add some proper URL recognition code...
            ...
            if( args.indexOf( "://") >= 0 ) {
                ...
                read.OpenLSM( args, true );

            } else {

                // System.out.println( "Doesn't look like a URL." );
    ...
    class GUI {
        ...
        private int baseFrameXlocation = (int)((ScreenX) - (baseFrameXsize));
        private int baseFrameYlocation = (int)((ScreenY) - (baseFrameYsize) - 30);
        private int detailsFrameXlocation = 0;
        private int detailsFrameYlocation = infoFrameYsize;
        ...
        public void init() {
            ...
            baseFrame.setTitle("LSM Reader 3.2f");
        ...
        public void initInfoFrame() {
            ...
            JButton dumpinfos_button = new JButton("Dump to text file");
            addDumpInfosListener(dumpinfos_button, infoFrame);
            infoFrame.getContentPane().add(infopanel,BorderLayout.NORTH);
            infoFrame.getContentPane().add(notes_button,BorderLayout.EAST);
            infoFrame.getContentPane().add(dumpinfos_button,BorderLayout.SOUTH);
        ...
        /***********************************************************************
         * method : dumpInfo dumps the Show Infos data in a textwindow *
         **********************************************************************/
        ...
        public void dumpInfo(){
            ...
            for (int i=0;i<19;i++) tw.append(Parameters[i]+"\t"+infos[i]);
        }
        ...
        private void addDumpDataListener(final JButton button, final JFrame parent) {
          ...
          /** Adds the "Dump Infos" function to a JButton in a JFrame */
    ...
    public void applyZSTAMP(ImagePlus imp,LSMFileInfo lfi){
        ...
        if (lfi.DIMENSION_Z!=1)
            ...
            for (int slice=1; slice<=lfi.DIMENSION_Z; slice++) {
                ...
                ip.setColor(c.getHSBColor(255,255,255-hsb[2]));
                ip.moveTo(x, y);
                ip.drawString(s);
            }
            } else
    ...
    class MyFocusListener extends WindowAdapter {
        ...
        public void windowActivated(WindowEvent e) {
            ...
            if (theGUI != null) {
                theGUI.lsm_fi = lfi;
                if (theGUI.lsm_fi.TIMESTACKSIZE<=1)
                    theGUI.butt5.setEnabled(false);
                else
                    theGUI.butt5.setEnabled(true);
                if (theGUI.lsm_fi.DIMENSION_Z<=1)
                    theGUI.butt7.setEnabled(false);
                else
                    theGUI.butt7.setEnabled(true);
                if (theGUI.lsm_fi.SPECTRALSCAN==0)
                    theGUI.butt8.setEnabled(false);
                else if (theGUI.lsm_fi.LS_COUNT>=1)
                    theGUI.butt8.setEnabled(true);
                ...
                if (theGUI.switcher == true)
                    theGUI.SwitchButton.setText("Switch to general view");
                else
                    theGUI.SwitchButton.setText("Switch to filtered view");
            }
        }
        ...
        public void windowLostFocus(WindowEvent e){
            ...
            if (theGUI != null) {
                ...
                if (theGUI.lsm_fi.DIMENSION_Z==1)
                    ...
                    theGUI.butt7.setEnabled(true);
                if (theGUI.lsm_fi.SPECTRALSCAN==1 && theGUI.lsm_fi.LS_COUNT>1)
                    theGUI.butt8.setEnabled(false);
                else
                    theGUI.butt8.setEnabled(true);
                theGUI.updateInfoFrame(theGUI.printINFO());
    ...
    class LSM_Reader_ {
        ...
        public void OpenLSM( String arg, boolean arg_is_URL ){

            // System.out.println( "OpenLSM(String,boolean); arg: " + arg + ", arg_is_URL: " + arg_is_URL );
            ...
            try {
                ...
                if( arg_is_URL ) {
                    ...
                    stream = new RandomAccessStream( is );
                    ...
                    File LSMthowAway = new File(DIRECTORY, FILENAME);
                    FileInputStream fis = new FileInputStream(LSMthowAway);
                    file = null;
                    stream = new RandomAccessStream(fis);
                ...
                /* Tests if the selected file is a valid LSM 510 file */
                    ...
                    if (test){
                        ...
                        //Vector v=new Vector();
                        //v.
                        // IJ.log (IJ.d2s(lsm_fi.IMAGETYPE.size(),0));
                        ...
                        if (lsm_fi.OFFSET_TIMESTAMPS!=0) {
                            ...
                            /* Searches for events marked in time-series and displays them in a scrollable window */
                            if ( (lsm_fi.SCANTYPE==3) ||(lsm_fi.SCANTYPE==4)||(lsm_fi.SCANTYPE==5) ||(lsm_fi.SCANTYPE==6)||(lsm_fi.SCANTYPE==9)) {
                                getEVENTLIST((int)lsm_fi.OFFSET_EVENTLIST );
                            }
                    ...
                    //int daOffset = (lsm_fi.DATATYPE == 2)?((int) (lsm_fi.TIF_IMAGEWIDTH*lsm_fi.TIF_IMAGELENGTH*2)):((int) (lsm_fi.TIF_IMAGEWIDTH*lsm_fi.TIF_IMAGELENGTH));
                        ...
                        for (int j = 0; j<(int)(lsm_fi.NUMBER_OF_CHANNELS); j++){
                            ...
                            imp = open(DIRECTORY, FILENAME,lsm_fi.STRIPOFF,j,true);
        ...
        public void OpenLSM(String arg1, String arg2){
            ...
            // System.out.println( "OpenLSM(String,String); arg1: " + arg1 + ", arg2: " + arg2 );
                ...
                /* Tests if the selected file is a valid LSM 510 file */
                    ...
                    if (test) {
                        ...
                        for (int j = 0; j<(int)(lsm_fi.NUMBER_OF_CHANNELS); j++){
                            ...
                            ImagePlus imp = open(DIRECTORY, FILENAME,lsm_fi.STRIPOFF,j,true);
                            imp.setTitle(FILENAME + " Channel : " + lsm_fi.Channel[j]);
         ...
         * image data and sends them to an array for batch conversion.     *
         ****************************************************************/
        ...
        public ImagePlus[] OpenLSM2(String arg1, String arg2){
                ...
                /* Tests if the selected file is a valid LSM 510 file */
                    ...
                    if (test){
                        ...
                        for (int j = 0; j<(int)(lsm_fi.NUMBER_OF_CHANNELS); j++){
                            ...
                            ImagePlus imp = open(DIRECTORY, FILENAME,lsm_fi.STRIPOFF,j,false);
                            // imp.setTitle(FILENAME + " Channel : " + lsm_fi.Channel[j]);
            ...
            int TotalLength = Total.size();
            ImagePlus[] impTotal = new ImagePlus[TotalLength];
            for (int n=0; n<TotalLength; n++) impTotal[n] = (ImagePlus) Total.get(n);
            return impTotal;
        }
        ...
        public void getCZ_LSMINFO(long position) {
            try {
                ...
                stream.seek((int)position+136);
                lsm_fi.OFFSET_EVENTLIST = swap(stream.readInt());
        ...
        /*****************************************************
         * method : getEVENTLIST, gets time events from file *
         ****************************************************/

        public void getEVENTLIST(long position) {

            String header = new String("Time (sec) \tEvent Type \tEvent Description");
            TextWindow tw = new TextWindow("Time Events for "+lsm_fi.FILENAME,header,null,400, 200);

            long EventTypeCode = 0;
            String EventType = "";
            double [] EventTime;
            ...
            try {
                stream.seek((int)position);
                EL_BYTESIZE = swap(stream.readInt());
                EL_COUNT = swap(stream.readInt());
                if (EL_COUNT>0) {
                    EventTime = new double [(int)EL_COUNT];
                    ...
                    for (int i = 0;i < EL_COUNT;i++)
                    {
                        ...
                        EV_SIZE = swap(stream.readInt());
                        EventTime [i] = swap(stream.readDouble());

                        /* time for the event is now determined */
                        EventDescription += IJ.d2s(EventTime[i]-lsm_fi.FirstTimeStamp) + "\t";

                        EventTypeCode = swap(stream.readInt());

                        switch ((int)EventTypeCode) {
                            case (0):
                                EventType = "Marker";
                                break;
                            case (1):
                         ...
                         /* Type of the event is now determined */
                        EventDescription += EventType + "\t";

                        LineSize = (int) EV_SIZE-16;
                        if (LineSize<255){
                            if( file != null )
                                file.seek(stream.getFilePointer());
                            if (LineSize>1) {
                                if( file != null )
                                    EventNote = file.readLine();
                                else
                                    EventNote = LSM_Reader.readLine(stream);
                            }
                            stream.seek(Pointer+(int)EV_SIZE);
                            if (EventNote.length()>5) EventNote = EventNote.substring(4,LineSize-1);
                            if (LineSize>5) EventNote = EventNote.trim();
                            if (EventNote.length()==0) EventNote=("");
                        }
                        /* Description of the event is now complete */
                        EventDescription += EventNote + "\t";

                        tw.append(EventDescription);
                        lsm_fi.TimeEvents += "- Description :" + EventDescription + "\n\n";
        ...
        public void getTIMESTAMPS(long position) {
            try {
                ...
                lsm_fi.FirstTimeStamp = lsm_fi.TS_STAMPS[0];
        ...
        public void getCHANNELNAMESANDCOLORS(long position){
            try{
                ...
                if( file != null )
                    ...
                    stream.seek(NamesOffset + (int) position );

                int Namesize = BlockSize - NamesOffset;

                String AllNames;
                if( file != null )
                    AllNames = file.readLine();
                else
                    AllNames = LSM_Reader.readLine(stream);
        ...
        public ImagePlus open(String directory, String file,Vector offsets ,int channel_iterator,boolean showImages) {

            // n.b. file and directory might be empty here if we're
            // loading from a URL...
            ...
            try {
                ...
                if( this.file == null ) {
                    ...
                    // Then we don't have a real file that we're loading from...

                    stream.seek( 0 );
                    // System.out.println( "stream starts at: " + stream.getFilePointer() );
                    ...
                    for (int k=0;k<v1.size();k++){
                        stream.skip(skip);
                        // System.out.println( "skipped " + skip + " and now reading from " + stream.getFilePointer() + " with k = " + k );
