...
public class PCA_Registration implements PlugIn {
    ...
    public PrincipalComponents doPCA( ImagePlus image, Threshold threshold, boolean drawAxes ) {
        ...
        try {
            ...
            for( int j = 0; j < vectorLength; ++j ) {
                System.out.println( "The sum of dimension " + j + " was " + sum[j] );
            }
            ...
            for( int j = 0; j < vectorLength; ++j ) {
                ...
                System.out.println( "The mean of dimension " + j + " was " + mean[j] );
            }

            // Calculate the variance of each dimension, and the covariance matrix.
    ...
    public int aboveThresholdLengthAlong( PrincipalComponents pcaResults,
        ...
        double [] v_unit = FastMatrix.normalize( v );
    ...
    public void chooseStacks() {
        ...
        FastMatrix translateDomainToMean = new FastMatrix( tmpTranslateDomain ).plus( new FastMatrix(1) );

        FastMatrix translateDomainBackFromMean = new FastMatrix( tmpTranslateDomain ).scale(-1,-1,-1).plus( new FastMatrix(1) );

        double [][] tmpTranslateTemplate = { { 0, 0, 0, -templatePCs.meanXYZ[0] },
                                             { 0, 0, 0, -templatePCs.meanXYZ[1] },
                                             { 0, 0, 0, -templatePCs.meanXYZ[2] } };

        FastMatrix translateTemplateToMean = new FastMatrix( tmpTranslateTemplate ).plus( new FastMatrix(1) );
        FastMatrix translateTemplateBackFromMean = new FastMatrix( tmpTranslateTemplate ).scale(-1,-1,-1).plus( new FastMatrix(1) );

        FastMatrix finalTranslationsMatrix = FastMatrix.translate( templatePCs.meanXYZ[0] - domainPCs.meanXYZ[0],
        ...
        FastMatrix [] rotations = new FastMatrix[4];

        rotations[0] = FastMatrix.rotateToAlignVectors( v2_template,
                                                        v1_template,
                                                        v2_domain,
                                                        v1_domain );
        ...
        rotations[1] = FastMatrix.rotateToAlignVectors( v2_template,
                                                        v1_template,
                                                        v2_domain,
                                                        v1_domain );
        ...
        rotations[2] = FastMatrix.rotateToAlignVectors( v2_template,
                                                        v1_template,
                                                        v2_domain,
                                                        v1_domain );
        ...
        rotations[3] = FastMatrix.rotateToAlignVectors( v2_template,
                                                        v1_template,
                                                        v2_domain,
                                                        v1_domain );
        ...
        if( (lengthIn0 == 0) || (lengthIn1 == 0) )
            ...
            scaling = new FastMatrix( 1.0 );
        else
            scaling = new FastMatrix( scaleFactor );
       ...
       // Now chain these together...
        ...
        transformations[bestIndex].createNewImage( sourceImages[0].getStack(), sourceImages[1].getStack(), true );
