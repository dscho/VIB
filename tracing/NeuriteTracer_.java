...
public class NeuriteTracer_ extends ThreePanes implements PlugIn {
...
*/	
	...
	Connection bestAStarBetween( int start_x, int start_y, int start_z,
				     int goal_x, int goal_y, int goal_z,
				     boolean reciprocal, double timeoutSeconds ) {
		...
		while( (open_from_start.size() > 0) && (open_from_goal.size() > 0) ) {
			...
			if( 0 == (loops % 5000) ) {
				...
				if( (timeoutSeconds > 0) && ((System.currentTimeMillis() - started_at) > (1000 * timeoutSeconds)) )
					break;
			}
