...
/* This is based on AStarThread, but certain things have been moved to
 * subclasses now.  For example, instead of start_* and goal_* we set
 * up those separately.  The complete list of these is:

   replace with "set start" and "set goals" method (or something
   similar):
   ...
   details of how to calculate the distances between points are now in
   the SearchNode class (and subclasses)

      boolean reciprocal;

      ComputeCurvatures hessian;

   subclasses should implement a getResult if they need it, so no:

      Path result

   costMovingTo should be implemented by a subclass

   estimateCostToGoal should be implemented by a subclass

   reciprocal fudge needs to move down
...
public abstract class SearchThread extends Thread {
	...
	// FIXME: The mechanism for reporting things at various
	// intervals will probably have to change.
	...
	// Safely stops the thread (for discarding the object.)
		...
		System.out.println("requestStop finished");
        }
		...
		*/
			...
			// whether we should report the progress, etc.
				...
				long since_last_report = currentMilliseconds - lastReportMilliseconds;
					...
					System.out.println( "milliseconds per loop: " +
							...
							if( alreadyThere == null ) {
								...
								if( bidirectional ) {
									...
									if( (i == 0) && ((alreadyThere.searchStatus == OPEN_FROM_GOAL) ||
										...
										result = e.asPath();
                                                                                System.out.println("e.asPath() is: "+e.asPath());
                                                                                Path fromGoalReversed = alreadyThere.asPathReversed();
                                                                                System.out.println("fromGoalReversed is: "+fromGoalReversed);
										result.add( fromGoalReversed );
                                                                                System.out.println("added, that is: "+result);
										done = true;
										...
										result = alreadyThere.asPath();
                                                                                System.out.println("alreadyThere.asPath() is "+alreadyThere.asPath());
                                                                                System.out.println("now the path from goal reversed is: "+e.asPathReversed());
